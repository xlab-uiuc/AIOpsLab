...
async def compose(addr, nodes, limit=200):
    ...
    async with aiohttp.ClientSession(connector=conn) as session:
        ...
        for i in range(nodes):
            for _ in range(
                random.randint(0, 20)
            ):  # up to 20 posts per user, average 10
                task = asyncio.ensure_future(
                    upload_compose(session, addr, i + 1, nodes)
                )
                tasks.append(task)
                idx += 1
                if idx % limit == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--graph",
        help="Graph name. (`socfb-Reed98`, `ego-twitter`, or `soc-twitter-follows-mun`)",
        default="socfb-Reed98",
    )
    parser.add_argument(
        "--ip",
        help="IP address of socialNetwork NGINX web server. ",
        default="127.0.0.1",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compose",
        action="store_true",
        help="intialize with up to 20 posts per user",
        default=False,
    )
    parser.add_argument(
        "--limit", type=int, help="total number simultaneous connections", default=200
    )
    args = parser.parse_args()

    with open(
        os.path.join("datasets/social-graph", args.graph, f"{args.graph}.nodes"), "r"
    ) as f:
        nodes = getNumNodes(f)
    with open(
        os.path.join("datasets/social-graph", args.graph, f"{args.graph}.edges"), "r"
    ) as f:
        edges = getEdges(f)
