...
def copy_and_execute_script(username, private_key, public_ip, script):
    ...
    try:
        ...
        run_command(["scp", "-o", "StrictHostKeyChecking=no", "-i", private_key, script, remote_path])

        # Execute the shell script on the remote VM
        run_command(["ssh", "-i", private_key, f"{username}@{public_ip}", f"bash /home/{username}/{os.path.basename(script)}"])
    except Exception as e:
...
def get_kubeadm_join_remote(username, private_key, public_ip):
    ...
    generate_join_command = [
        "ssh", "-i", private_key, f"{username}@{public_ip}",
...
def run_kubeadm_join_on_worker(worker_username, private_key, worker_ip, join_command):
    ...
    ssh_command = [
        "ssh", "-i", private_key, f"{worker_username}@{worker_ip}", 
...
def deploy_prometheus(username, private_key_file_1, public_ip_1):
    ...
    try:
        run_command(["ssh",  "-o", "StrictHostKeyChecking=no", "-i", private_key_file_1, f"{username}@{public_ip_1}", f"bash {REPO}/scripts/setup.sh kubeworker1"])
    except Exception as e:
